...
exports.listen = function (server) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        handleChatAttempts(socket, nickNames, namesUsed);
...
var joinRoom =  function (socket, room) {
    ...
    if (usersInRoom.length > 1) {
        ...
        for (var item in usersInRoom) {
            var userSocketId = usersInRoom[item].id;
...
var handleClientDisconnection  = function (socket) {
    socket.on('disconnect', function () {
       var nameIndex = nameUsed.indexOf(niknames[socket.id]);
